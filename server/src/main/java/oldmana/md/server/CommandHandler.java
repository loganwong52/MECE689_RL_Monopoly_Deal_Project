...
public class CommandHandler {
	...
	public void registerDefaultCommands() {
		...
		registerCommand(new CommandEditDeck());
		registerCommand(new CommandAddRLBot());

		// Registering test/debug commands through reflection, as they're only used in a
	...
	public void executeCommand(CommandSender sender, String fullCmd) {
		...
		if (sender instanceof Player) {
			System.out.println(
					((Player) sender).getName() + " issued " + (cmd == null ? "unknown " : "") + "command: " + fullCmd);
		}
		if (cmd == null) {
